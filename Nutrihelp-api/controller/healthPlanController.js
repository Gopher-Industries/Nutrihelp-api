...
const { fetchAi } = require("../services/aiHttpClient");

const AI_BASE =
  process.env.AI_BASE_URL || "https://localhost:8000/ai-model/medical-report";
...
function buildHealthGoalFromSurvey(survey) {
  ...
  const dpw = dpwRaw === undefined || dpwRaw === null || dpwRaw === "" ? 3 : Number(dpwRaw);
...
const generateWeeklyPlan = async (req, res) => {
  ...
  try {
    ...
    // call AI
    const aiResponse = await fetchAi(`${AI_BASE}/plan/generate`, {
      method: "POST",
