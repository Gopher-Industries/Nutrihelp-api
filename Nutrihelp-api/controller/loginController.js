...
const { tokenHookOnIssue, TOKEN_TYPES } = require("../services/tokenLogService");
...
function triggerAlertCheck(context) {
  checkAlerts({ send: true, source: "login-controller", context }).catch((error) => {
    console.error("[loginController] Alert check failed:", error.message || error);
  });
}
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (!userExists) {
      ...
      triggerAlertCheck({ event: "login_user_not_found", email, ip: clientIp });
    ...
    if (!isPasswordValid) {
      ...
      triggerAlertCheck({ event: "login_invalid_password", email, ip: clientIp });
    ...
    triggerAlertCheck({ event: "login_success", email, userId: user.user_id, ip: clientIp });
...
const loginMfa = async (req, res) => {
  ...
  try {
    ...
    await sessionHookOnLoginSuccess(req, user);
    await tokenHookOnIssue(req, user.user_id, TOKEN_TYPES.ACCESS);
    triggerAlertCheck({ event: "login_mfa_success", email, userId: user.user_id, ip: req.ip });
