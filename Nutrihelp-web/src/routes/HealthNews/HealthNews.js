...
const SEARCH_ARTICLES_FUNCTION_URL =
  process.env.REACT_APP_SEARCH_ARTICLES_FUNCTION_URL ||
  'https://mdauzoueyzgtqsojttkp.supabase.co/functions/v1/search-articles';
...
const HealthNews = () => {
  ...
  const searchArticles = async () => {
    ...
    try {
      if (!SEARCH_ARTICLES_FUNCTION_URL.startsWith('https://')) {
        throw new Error('Search endpoint must use HTTPS.');
      }

      const response = await fetch(`${SEARCH_ARTICLES_FUNCTION_URL}?q=${encodeURIComponent(apiSearchQuery)}`);
