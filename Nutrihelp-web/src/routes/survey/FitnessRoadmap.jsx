...
export default function FitnessRoadmap() {
  ...
  useEffect(() => {
    ...
    const surveyStored = localStorage.getItem('ObesitySurveyData');
    if (!stored) {
    ...
    if (!surveyStored) {
      setError('⚠️ Missing survey data. Please complete the questionnaire again.');
      setLoading(false);
      return;
    }
    const parsed = JSON.parse(stored);
    const surveyData = JSON.parse(surveyStored);

    fetch(apiUrl('/medical-report/plan'), {
      method: 'POST',
