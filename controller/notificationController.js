...
exports.deleteNotificationById = async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('notifications').delete().eq('simple_id', id); // Only delete the notification with the specific id
...
exports.markAllUnreadNotificationsAsRead = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: 'All unread notifications marked as read', updatedNotifications: data });
  } catch (error) {
