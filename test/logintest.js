  ...
  it('should return 400 Email and password are required', (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.have.property('error').that.equals('Email and password are required');
        done();
      });
  });
});
  ...
  it('should return 401 Invalid email', (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.have.property('error').that.equals('Invalid email');
        done();
      });
  });
});
  ...
  it('should return 401 Invalid password', (done) => {
    chai
      ...
      .end((err, res) => {
        if (err) return done(err);
        expect(res).to.have.status(401);
        expect(res.body).to.have.property('error').that.equals('Invalid password');
        done();
      });
  });
});
