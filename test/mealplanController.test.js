...
describe('Meal plan controller', () => {
  ...
  it('uses only the authenticated identity for /me even when another user_id is supplied', async () => {
    const getForAuthenticatedUser = sinon.stub().resolves([]);
    ...
    await controller.getMyMealPlan(req, res);

    expect(
      getForAuthenticatedUser.calledOnceWith(
        15,
        {
  ...
  it('keeps User A and User B meal-plan queries isolated', async () => {
    const getForAuthenticatedUser = sinon.stub().resolves([]);
    ...
    await controller.getMyMealPlan(
      {
        user: { userId: 101, role: 'user' },
        query: {},
      },
      makeRes()
    );
    ...
    await controller.getMyMealPlan(
      {
        user: { userId: 202, role: 'user' },
        query: {},
      },
      makeRes()
    );

    expect(getForAuthenticatedUser.callCount).to.equal(2);
    expect(getForAuthenticatedUser.firstCall.args[0]).to.equal(101);
    expect(getForAuthenticatedUser.secondCall.args[0]).to.equal(202);
  });
  ...
  it('rejects an invalid date range before querying the database', async () => {
    ...
    expect(getForAuthenticatedUser.called).to.equal(false);
