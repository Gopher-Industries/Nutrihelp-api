...
async function createRecipe(
  ...
  try {
    let { data, error } = await supabase.from('ingredients').select('*').in('id', ingredient_id);
...
async function saveRecipe(recipe) {
  try {
    let { data, error } = await supabase.from('recipes').insert(recipe).select();
    return data;
  } catch (error) {
    throw error;
  }
...
async function saveImage(image, recipe_id) {
  ...
  try {
    ...
    let { data: image_data } = await supabase.from('images').insert(test).select('*');
...
async function saveRecipeRelation(recipe, savedDataId) {
  try {
    ...
    let { data, error } = await supabase.from('recipe_ingredient').insert(insert_object).select();
