...
async function updateUserPreferences(userId, body) {
  try {
    ...
    const { error: aError } = await supabase.from('user_allergies').delete().eq('user_id', userId);
    if (aError) throw aError;

    const { error: cError } = await supabase.from('user_cuisines').delete().eq('user_id', userId);
    if (cError) throw cError;

    const { error: dError } = await supabase.from('user_dislikes').delete().eq('user_id', userId);
    ...
    const { error: driError } = await supabase
      .from('user_dietary_requirements')
      .insert(
        body.dietary_requirements.map((id) => ({ user_id: userId, dietary_requirement_id: id }))
      );
