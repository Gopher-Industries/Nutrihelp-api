...
| **code-coverage** | ≥50% test coverage | ❌ Blocks merge |
...
3. Enable:
   ...
   - ✅ Select all checks: `lint`, `format-check`, `unit-tests`, `openapi-validate`, `security-scan`, `build-check`, `code-coverage`
...
gh api repos/Gopher-Industries/Nutrihelp-api/branches/main/protection \
  --method PUT \
  -f required_status_checks='{"strict":true,"contexts":["lint","format-check","unit-tests","openapi-validate","security-scan","build-check","code-coverage"]}' \
